...
impl Args {
    pub fn read_secret(&self) -> Result<Signer, Error> {
        if let Ok(secret_key) = std::env::var("SOROBAN_SECRET_KEY") {
            Ok(Signer::SecretKey { secret_key })
...
impl From<PrivateKey> for Signer {
...
impl Signer {
    pub fn private_key(&self, index: Option<usize>) -> Result<PrivateKey, Error> {
        Ok(match self {
            Signer::SecretKey { secret_key } => PrivateKey::from_string(secret_key)?,
            Signer::SeedPhrase { seed_phrase } => PrivateKey::from_payload(
                &sep5::SeedPhrase::from_str(seed_phrase)?
                    .from_path_index(index.unwrap_or_default(), None)?
                    .private()
                    .0,
            )?,
            Signer::Ledger => panic!("Ledger does not reveal secret key"),
        })
    }
    ...
    pub fn signer(&self, index: Option<usize>, prompt: bool) -> Result<StellarSigner, Error> {
        match self {
            Signer::SecretKey { .. } | Signer::SeedPhrase { .. } => Ok(StellarSigner::Local(
    ...
    pub fn from_seed(seed: Option<&str>) -> Result<Self, Error> {
        ...
        .seed_phrase
        .into_phrase();
        Ok(Signer::SeedPhrase { seed_phrase })
    }
