...
pub struct Cmd {
    ...
    /// Spender drawing on its allowance. Signs and authorizes the transfer, so
    /// it must be an identity or secret key you control.
    #[arg(long)]
...
/// The machine-readable receipt of a delegated token transfer.
#[derive(Debug, serde::Serialize)]
struct Receipt {
    /// Hex-encoded hash of the submitted transaction.
    tx_hash: Option<String>,
    /// The decoded contract return value (`null` for SEP-41 `transfer_from`,
...
impl Cmd {
    ...
    /// `--spender` as the source account that signs and authorizes the transfer.
    ...
    pub async fn run(&self, global_args: &global::Args) -> Result<(), Error> {
        ...
        // SEP-41 `transfer_from(spender, from, to, amount)`: `spender` is the
        // source account (which also signs and authorizes) drawing on its
        // allowance, `from` is the owner whose funds move, `to` is the
        // destination.
        //
        ...
        // SEP-41 `transfer_from(spender, from, to, amount)` — supply the values
        // in that order and let the contract's parameters be matched by
        // position, so a token that names them anything still works. A transfer
        // always intends to submit, so force `Send::Yes`: a token whose
        // `transfer_from` records no writes/events/auth can't be classified
        ...
        // `transfer_from` always writes, so the invocation is submitted rather
