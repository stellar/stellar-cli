...
use crate::{
    ...
    config::{self, locator, network, sign_with, token::UnresolvedToken, UnresolvedMuxedAccount},
    output::Output,
};
...
#[group(skip)]
pub struct Cmd {
    /// The token to burn: a contract id or alias, `native`, or a classic asset
    /// as `CODE:ISSUER`.
    #[arg(long = "id")]
    pub id: UnresolvedToken,

    /// Account whose tokens are destroyed. Signs and authorizes the burn, so it
...
/// The machine-readable result of a token burn.
#[derive(Debug, serde::Serialize)]
struct BurnResult {
    /// Hex-encoded hash of the submitted transaction.
    tx_hash: Option<String>,
    /// The decoded contract return value (`null` for SEP-41 `burn`, which
    /// returns nothing).
...
impl Cmd {
    ...
    /// `--from` as the source account that signs and authorizes the burn. Fees
    ...
    pub async fn run(&self, global_args: &global::Args) -> Result<(), Error> {
        ...
        // SEP-41 `burn(from, amount)`: `from` is the source account (which also
        // signs and authorizes), whose tokens are destroyed.
        //
        // The invoke pipeline can't source a transaction from a muxed account
        // yet (see #2645), and a muxed strkey in the `burn` arg is rejected
        ...
        // SEP-41 `burn(from, amount)` — supply the values in that order and let
        // the contract's parameters be matched by position, so a token that
        // names them anything still works. A burn always intends to submit, so
        // force `Send::Yes`: a token whose `burn` records no writes/events/auth
        // can't be classified read-only and silently exit 0 without ever
        // destroying funds.
        ...
        // `burn` always writes, so the invocation is submitted rather than
