...
mod tests {
    ...
    use soroban_spec_markers::{
        generate_graph_record, generate_spec_id_for_xdr, SpecGraphEntryKind, GRAPH_SECTION,
        META_KEY, META_VALUE_V2,
    };
    ...
    fn shakes_v2_wasm_and_removes_sidecar() {
        let wasm = wasm_with_sections(Some(META_VALUE_V2), true);
        ...
        assert!(!section_names.contains(&GRAPH_SECTION.to_string()));
    ...
    fn wasm_with_sections(meta_version: Option<&str>, include_graph: bool) -> Vec<u8> {
        ...
        wasm_gen::write_custom_section(&mut wasm, CONTRACT_SPEC_SECTION, &spec);
        if include_graph {
            wasm_gen::write_custom_section(&mut wasm, GRAPH_SECTION, &graph_record(&spec));
        }
        wasm
    }

    fn graph_record(spec_xdr: &[u8]) -> Vec<u8> {
        generate_graph_record(
            SpecGraphEntryKind::Function,
            generate_spec_id_for_xdr(spec_xdr),
            &[],
        )
    }
