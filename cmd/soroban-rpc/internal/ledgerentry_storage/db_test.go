...
func assertLedgerRange(t *testing.T, db DB, start, end uint32) {
	allLedgers, err := db.GetAllLedgers()
	assert.NoError(t, err)
	for i := start - 1; i <= end+1; i++ {
		ledger, exists, err := db.GetLedger(i)
		assert.NoError(t, err)
		if i < start || i > end {
...
func TestLedgers(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		assert.NoError(t, err)
		assert.NoError(t, tx.InsertLedger(createLedger(ledgerSequence)))
		assert.NoError(t, tx.Done())
	}

	assertLedgerRange(t, db, 1, 10)

	ledgerSequence := uint32(11)
	tx, err := db.NewLedgerEntryUpdaterTx(ledgerSequence, 150)
	assert.NoError(t, err)
	assert.NoError(t, tx.InsertLedger(createLedger(ledgerSequence)))
	assert.NoError(t, tx.TrimLedgers(15))
	assert.NoError(t, tx.Done())

	assertLedgerRange(t, db, 1, 11)

	ledgerSequence = uint32(12)
	tx, err = db.NewLedgerEntryUpdaterTx(ledgerSequence, 150)
	assert.NoError(t, err)
	assert.NoError(t, tx.InsertLedger(createLedger(ledgerSequence)))
	assert.NoError(t, tx.TrimLedgers(5))
	assert.NoError(t, tx.Done())

	assertLedgerRange(t, db, 8, 12)
}
