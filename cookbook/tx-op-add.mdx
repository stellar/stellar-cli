...
Let's consider a more complicated example. Consider issuing an asset, here `USDC` with the requirement that only the issuer can transfer funds to the distributor.
...
# Next is an example of sandwiching an operation. That is giving permission in one operation, performing the operation, and then removing the permission in a third operation.
