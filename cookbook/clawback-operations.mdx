...
**Important**: The Stellar CLI currently doesn't support direct clawback operations (CLI doesnt support clawback). This functionality requires:
...
When creating an asset, clawback must be enabled. This is typically done through custom contract deployment or direct Stellar SDK operations:
...
Deploy Stellar Asset Contract first, then deploy your clawback contract:
...
Set authorization required and freeze flags:
...
Freeze an account's asset balance, then unfreeze when needed:
