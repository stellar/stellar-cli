...
When deploying your contract, you can include metadata. Deploy contract with metadata file:
...
For already deployed contracts, add metadata through contract calls. Invoke contract to set metadata:
...
Check existing metadata for a contract. Get contract information including metadata:
...
Modify existing metadata. Update contract name and description:
...
Delete specific metadata entries. Remove a metadata key:
...
Update multiple metadata fields at once. Set multiple metadata fields:
...
Ensure metadata follows standards. Validate metadata format:
...
Add metadata to Stellar Asset Contracts. Deploy SAC with metadata:
...
```bash
# Generate token contract metadata template
stellar contract metadata template \
    --type token \
    --output token_metadata.json

# Generate DeFi contract metadata template
stellar contract metadata template \
    --type defi \
