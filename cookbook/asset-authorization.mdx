...
## Setting Up Your Issuer Account

Before running the authorization commands, you need to create and fund an issuer account:

Generate a new issuer account, fund it with XLM for transaction fees, and verify the account was created:
...
**Note**: The following examples assume you have already set up your issuer account as described in the "Setting Up Your Issuer Account" section above.
...
Note: Account flags are set through transactions and can be verified by checking transaction history or using Stellar Laboratory.
...
Set common flag combinations:

```bash
# Authorization required + revocable
stellar tx new set-options \
    --source-account issuer \
    --set-required \
    --set-revocable

# Authorization required + revocable + immutable
stellar tx new set-options \
    --source-account issuer \
    --set-required \
    --set-revocable \
...
# Authorization required + immutable
stellar tx new set-options \
    --source-account issuer \
    --set-required \
    --set-immutable
...
Get detailed account information and look for the "flags" section in the output:
...
Generate and fund issuer account, set authorization flags, and verify:
...
stellar keys address controlled_asset_issuer
```
...
Freeze an account, then freeze specific account:
...
1. **Flags Cannot Be Changed**
   ...
   Check transaction history to verify current flags:
...
Set up network, create issuer account, set authorization, and deploy asset contract:
...
Create high-security issuer, set all authorization flags, and deploy with full control:
...
Check current flag status and review recent transactions:

Check account address and transaction history:
