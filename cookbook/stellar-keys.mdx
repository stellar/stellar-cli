...
Run the following command to create a new keypair and save it under the alias named `test`:
...
Output:
```
✅ Key saved with alias test in ".config/soroban/identity/test.toml"
```
...
The file test.toml contains the seed phrase for your identity.
...
vim test.toml
```
Inside, you will see the seed phrase associated with your identity.


### 4. Derive the Secret Key

To display the secret key for the test identity:
...
To display the corresponding public key for the test identity:
```bash
...
At this point, the identity file test.toml is deleted, and the alias is no longer available in the CLI.
