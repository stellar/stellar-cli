...
**Usage:** `stellar token clawback [OPTIONS] --id <ID> --admin <ADMIN> --from <FROM> --amount <AMOUNT>`
...
- `--admin <ADMIN>` — The token's administrator. Signs and authorizes the clawback, so it must be an identity or secret key you control (the asset issuer for a Stellar Asset Contract)
